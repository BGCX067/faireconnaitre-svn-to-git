 ...
 * @package connectionengine
 */
class Sqlsrv_result_STH_CACHED extends Result_STH {
	// --- ASSOCIATIONS ---
	 ...
	 * @return void
	 */
	public function Sqlsrv_result_STH_CACHED($result, $typeAssosFields, $query, $nbrow=0 ){
		global $LOGGER;
		...
		// THREAD RESULT
		$this->row = $this->result[$this->indexRow];

		if ( $this->indexRow >=  sizeof($this->result)) { $this->EOF = true; }
		else { $this->EOF = false; }
